...
export class PostService {
  ...
  getPosts(postsPerPage, currentPage) {
    ...
    this.http.get<{ message: string, posts: any, maxCount: number }>(this.BACKEND_URL + queryParams)
      .pipe(map((postData) => {
        return {
    ...
    // this.http.post(this.BACKEND_URL, {'a':'lmao'},{})
  }
  ...
  deletePost(postId: string) {
    return this.http.delete<{ message: string }>(this.BACKEND_URL + '/' + postId)
    // .subscribe(res => {
  ...
  updatePost(post: Post, image: File | string) {
    ...
    if (typeof (image) === 'object') {
      ...
      postData = { ...post, imagePath: image, creator: null };
    }
    this.http.put<{ message: string }>(this.BACKEND_URL + '/' + post.id, postData)
      .subscribe((res) => {
